...
function run_surrogate_model()
    ...
    # num_runs = num_initial_runs + num_additional_runs + num_swarm_runs * num_swarm_particles
    ...
    for i = 1:num_initial_runs
        incidence_arr[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["observed_cases"]
        duration_parameter[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["duration_parameter"]
        susceptibility_parameters[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["susceptibility_parameters"]
        temperature_parameters[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["temperature_parameters"]
        mean_immunity_durations[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["mean_immunity_durations"]
        random_infection_probabilities[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld2"))["random_infection_probabilities"]
    end

    for i = 1:num_additional_runs
        incidence_arr[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["observed_cases"]
        duration_parameter[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["duration_parameter"]
        susceptibility_parameters[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["susceptibility_parameters"]
        temperature_parameters[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["temperature_parameters"]
        mean_immunity_durations[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["mean_immunity_durations"]
        random_infection_probabilities[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "mcmc", "results_$(i).jld2"))["random_infection_probabilities"]
    end

    # for i = 1:num_additional_runs
    #     incidence_arr[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["observed_cases"]
    #     duration_parameter[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["duration_parameter"]
    #     susceptibility_parameters[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["susceptibility_parameters"]
    #     temperature_parameters[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["temperature_parameters"]
    #     mean_immunity_durations[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["mean_immunity_durations"]
    #     random_infection_probabilities[i + num_initial_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(i).jld2"))["random_infection_probabilities"]
    # end

    # for i = 1:num_swarm_particles
    #     for j = 1:num_swarm_runs
    #         incidence_arr[(i - 1) * num_swarm_runs + j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["observed_cases"]
    #         duration_parameter[i * j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["duration_parameter"]
    #         susceptibility_parameters[i * j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["susceptibility_parameters"]
    #         temperature_parameters[i * j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["temperature_parameters"]
    #         mean_immunity_durations[i * j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["mean_immunity_durations"]
    #         random_infection_probabilities[i * j + num_initial_runs + num_additional_runs] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(i)", "results_$(j).jld2"))["random_infection_probabilities"]
    #     end
    # end
    ...
    #         swarm_incidence = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["observed_cases"]
    ...
    #         par_vec[1] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["duration_parameter"]
    #         par_vec[2:8] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["susceptibility_parameters"]
    #         par_vec[9:15] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["temperature_parameters"]
    #         par_vec[16:22] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["mean_immunity_durations"]
    #         par_vec[23:26] = load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["random_infection_probabilities"]
    #         r = reshape(par_vec, :, 1)

    #         # nMAE, _ = Lux.apply(lux_model, r, params, opt_state)

    #         # real_nMAE = sum(abs.(load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["observed_cases"] - num_infected_age_groups_viruses)) / sum(num_infected_age_groups_viruses)
    ...
    #         # y_model = vec(load(joinpath(@__DIR__, "..", "output", "tables", "swarm", "$(particle_number)", "results_$(i).jld2"))["observed_cases"])
    ...
    for curr_run = 1:500
        ...
        @time observed_num_infected_age_groups_viruses, activities_infections, rt, num_schools_closed = run_simulation(
            nothing, num_threads, thread_rng, agents, viruses, households, schools, duration_parameter_min,
        ...
        save(joinpath(@__DIR__, "..", "output", "tables", "surrogate", "results_$(curr_run).jld2"),
