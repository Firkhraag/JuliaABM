...
function run_surrogate_model()
    ...
    for i = 1:num_runs
        incidence_arr[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld"))["observed_cases"] ./ 10072
        duration_parameters[i] = load(joinpath(@__DIR__, "..", "output", "tables", "lhs", "initial", "results_$(i).jld"))["duration_parameter"]
    ...
    # parameters = [0.1 for i = 1:(num_parameters + 1)]
    # X = cat(X, [1.0 for i = 1:length(y)], dims = 2)
    # parameters = linear_regression(X, y, 0.001, parameters, 10000)
    # sse = transpose((X * parameters - y)) * (X * parameters - y)
    # println(sse)

    # model = Chain(Dense(num_parameters => round(Int, 2 / 3 * num_parameters), tanh), Dense(round(Int, 2 / 3 * num_parameters) => 1, bias=false), only)
    ...
    # pl = plot(x -> 2x-x^3, -3, 3, legend=false)
    # scatter!(x -> model([x]), -3:0.1f0:3)

    # savefig(pl, joinpath(@__DIR__, "test.pdf"))
