...
function run_ga_model()
    ...
    susceptibility_parameters_children = Vector{Float64}(undef, population_size)
    temperature_parameters_children = Vector{Float64}(undef, population_size)
    mean_immunity_durations_children = Vector{Float64}(undef, population_size)
    random_infection_probabilities_children = Vector{Float64}(undef, population_size)
