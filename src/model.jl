...
function global_sensitivity(
    ...
    save(joinpath(@__DIR__, "..", "output", "tables", "sensitivity", "results_$(run_num + starting_bias).jld2"),
...
function parameter_sensitivity(
    ...
    for m in multipliers
        # Моделируем заболеваемость
        @time observed_num_infected_age_groups_viruses, _, __, ___ = run_simulation(
            nothing, num_threads, thread_rng, agents, viruses, households, schools, duration_parameter * m,
...
function lhs_simulations(
    ...
    for i = (num_files + 1):num_runs
        ...
        save(joinpath(@__DIR__, "..", "output", "tables", "lhs", folder_name, "results_$(i).jld2"),
            "observed_cases", observed_num_infected_age_groups_viruses,
            "duration_parameter", duration_parameter,
            "susceptibility_parameters", susceptibility_parameters,
            "temperature_parameters", temperature_parameters,
...
function mcmc_simulations(
    ...
    for n = 1:1000
        ...
        save(joinpath(@__DIR__, "..", "output", "tables", "mcmc_$(sampling_type)", "results_$(n).jld2"),
            "observed_cases", observed_num_infected_age_groups_viruses,
            "duration_parameter", duration_parameter,
            "susceptibility_parameters", susceptibility_parameters,
            "temperature_parameters", temperature_parameters,
...
function run_model(
    ...
    ws::Union{WebSockets.WebSocket, Nothing} = nothing,
    ...
    # MCMC LHS 10
    # error = 1.478806749592584e10
    # duration_parameter = 0.10979573825043533,
    # susceptibility_parameters = [5.316681627071222, 2.5349602078491644, 3.217627754906059, 5.779848036568484, 4.906087864140951, 6.924991792237298, 4.541355331411498],
    # temperature_parameters = [-0.17789229569340134, -0.2413339602987009, -0.010155946794370495, -0.9473115531758222, -0.8095728771038602, -0.6327303856641534, -0.45640114252475766],
    # mean_immunity_durations = [335.1991521861829, 189.0333595657001, 362.56408769532646, 151.6389965790126, 104.8014998229103, 97.50793191673677, 233.28395255816622],
    # random_infection_probabilities = [0.001127516708800325, 0.0008929471208048208, 0.00026332601407402984, 6.8571466370070825e-6],
    ...
    # CGO_min
    # error = 9.895038907764772e9
    # duration_parameter = 0.10881361609607425,
    # susceptibility_parameters = [1.6468057552418598, 5.164503046355679, 5.901645025056615, 6.737774924712047, 3.887811754546221, 6.3237158138697245, 5.160771816833279],
    # temperature_parameters = [-0.9248445169445225, -0.7077058314385487, -0.12363686252658079, -0.10975084658020529, -0.15399602319820327, -0.6499951941949007, -0.8363511870013901],
    # mean_immunity_durations = [186.45869701608711, 116.95200284756245, 117.46454435287365, 284.8343516229486, 150.67748039442333, 52.28309255101259, 48.95950004009306],
    # random_infection_probabilities = [0.0009106915594893613, 0.000570231949727069, 0.0004475448437166233, 6.485176541309354e-6],

    # CGO
    # error = 1.0886224915637691e10
    # duration_parameter = 0.14829030822116515,
    # susceptibility_parameters = [3.0144069394271864, 6.887856005697344, 5.560618403916709, 6.319479845335145, 6.8796573502199285, 5.072476037502436, 5.302785026611835],
    # temperature_parameters = [-0.8778807125110569, -0.5553690304666298, -0.6456810965539765, -0.6761112736781968, -0.1785363361695398, -0.47683758448474667, -0.5994061169325967],
    # mean_immunity_durations = [77.93367381545295, 128.55268407796422, 117.02489370531795, 216.94738135469746, 61.19952081065328, 41.124016038083894, 204.11475138685995],
    # random_infection_probabilities = [0.0008185293382156444, 0.0005528414078060589, 0.0004363484549898493, 7.744259221234202e-6],
    ...
    if abs(global_warming_temperature) > 0.1
        ...
        save(joinpath(@__DIR__, "..", "output", "tables", "results_warming_$(run_num + 1).jld2"),
            "observed_cases", observed_num_infected_age_groups_viruses,
            "activities_cases", activities_infections,
            "rt", rt)
    elseif school_class_closure_period == 0
        # Базовый сценарий
        save(joinpath(@__DIR__, "..", "output", "tables", "results_$(run_num + 1).jld2"),
            "observed_cases", observed_num_infected_age_groups_viruses,
            "activities_cases", activities_infections,
            "rt", rt)
    else
        # Сценарий карантина в школах
        save(joinpath(@__DIR__, "..", "output", "tables", "results_quarantine_$(run_num + 1).jld2"),
            "observed_cases", observed_num_infected_age_groups_viruses,
            "activities_cases", activities_infections,
            "rt", rt,
