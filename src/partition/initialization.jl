function create_agent(
    ...
    if is_child
        if parent_age < 23
            # M0–4
            return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 1], rand(0:(parent_age - 18)))
        elseif parent_age < 28
            # T0-4_0–9
            if (age_rand_num <= district_people[index, 19])
                # M0–4
                Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 1], rand(0:4))
            else
                # M5–9
                Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 2], rand(5:(parent_age - 18)))
            end
        elseif parent_age < 33
            ...
            elseif (age_rand_num <= district_people[index, 21])
                # M5–9
                Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 2], rand(5:9))
            else
                # M10–14
                Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 3], rand(10:(parent_age - 18)))
            end
        elseif parent_age < 35
            ...
            if age_group_rand_num <= district_people_households[1, district_household_index]
                ...
                elseif (age_rand_num <= district_people[index, 21])
                    ...
                    # M10–14
                    Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 3], rand(10:14))
                end
            else
                # M15–19
                return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 4], rand(15:(parent_age - 18)))
            end
        else
            age_group_rand_num = rand(1:100)
            if age_group_rand_num <= district_people_households[1, district_household_index]
                ...
                # M15–19
                return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 4], rand(15:17))
            end
        end
    else
        age_group_rand_num = rand(1:100)
        ...
        if age_group_rand_num <= district_people_households[2, district_household_index]
            if is_male !== nothing
                ...
                # M20–24
                return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 5], rand(18:24))
            end
        elseif age_group_rand_num <= district_people_households[3, district_household_index]
            ...
            if age_rand_num <= district_people[index, 22]
                if is_male !== nothing
                    ...
                    # M25–29
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 6], rand(25:29))
                end
            else
                if is_male !== nothing
                    ...
                    # M30–34
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 7], rand(30:34))
                end
            end
        elseif age_group_rand_num <= district_people_households[4, district_household_index]
            ...
            if age_rand_num <= district_people[index, 23]
                if is_male !== nothing
                    ...
                    # M35–39
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 8], rand(35:39))
                end
            else
                if is_male !== nothing
                    ...
                    # M40–44
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 9], rand(40:44))
                end
            end
        elseif age_group_rand_num <= district_people_households[5, district_household_index]
            ...
            if age_rand_num <= district_people[index, 24]
                if is_male !== nothing
                    ...
                    # M45–49
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 10], rand(45:49))
                end
            else
                if is_male !== nothing
                    ...
                    # M50–54
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 11], rand(50:54))
                end
            end
        elseif age_group_rand_num <= district_people_households[6, district_household_index]
            ...
            if age_rand_num <= district_people[index, 25]
                if is_male !== nothing
                    ...
                    # M55–59
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 12], rand(55:59))
                end
            else
                if is_male !== nothing
                    ...
                    # M60–64
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 13], rand(60:64))
                end
            end
        else
            # T65-69_65–89
            if age_rand_num <= district_people[index, 26]
                if is_male !== nothing
                    ...
                    # M65–69
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 14], rand(65:69))
                end
            # T65-74_65–89
            elseif age_rand_num <= district_people[index, 27]
                if is_male !== nothing
                    ...
                    # M70–74
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 15], rand(70:74))
                end
            # T65-79_65–89
            elseif age_rand_num <= district_people[index, 28]
                if is_male !== nothing
                    ...
                    # M75–79
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 16], rand(75:79))
                end
            # T65-84_65–89
            elseif age_rand_num <= district_people[index, 29]
                if is_male !== nothing
                    ...
                    # M80–84
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 17], rand(80:84))
                end
            else
                if is_male !== nothing
                    ...
                    # M85–89
                    return Agent(agent_id, viruses, viral_loads, household_conn_ids, sex_random_num <= district_people[index, 18], rand(85:89))
                end
            end
        end
    end
...
function create_population(
    ...
    agent_id = 1
    for index in district_nums[comm_rank:comm_size:size(district_nums, 1)]
        ...
        for _ in 1:district_households[index, 1]
            ...
            agents = Agent[create_agent(
                agent_id, viruses, viral_loads, household_conn_ids, index, district_people, district_people_households, index_for_1_people)]
            agent_id += 1
        ...
        for _ in 1:district_households[index, 2]
            ...
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 0, 0, index)
        ...
        for _ in 1:district_households[index, 3]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 0, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 4]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 1, 0, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 5]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 0, 2, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 6]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 1, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 7]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 2, 0, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 8]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 3, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 9]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 1, 2, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 10]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 2, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 11]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 3, 0, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 12]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 4, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 13]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 1, 3, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 14]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 2, 2, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 15]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 3, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 16]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 0, 0, index)
            agent_id += 2
        ...
        for _ in 1:district_households[index, 17]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 1, index)
            agent_id += 3
            agents2 = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 0, index)
            append!(agents, agents2)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 18]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 1, 0, index)
            agent_id += 3
            agents2 = create_parents_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 0, index)
            append!(agents, agents2)
        ...
        for _ in 1:district_households[index, 22]
            ...
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 0, 1, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 23]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 1, 0, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 24]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 0, 2, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 25]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 1, 1, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 26]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 2, 0, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 27]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 0, 3, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 28]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 1, 2, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 29]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 2, 1, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 30]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 3, 0, index, false)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 31]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 0, 1, index, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 32]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 1, 0, index, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 33]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 0, 2, index, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 34]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 1, 1, index, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 35]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 2, 0, index, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 36]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 0, 2, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 37]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 1, 1, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 38]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 0, 3, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 39]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 1, 2, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 40]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 2, 1, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        ...
        for _ in 1:district_households[index, 46]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 0, 4, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 47]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 1, 3, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 48]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_parent_with_children(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_5_people, 2, 2, index, nothing, true)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        ...
        for _ in 1:district_households[index, 49]
            ...
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 0, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 50]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_2_people, 1, 0, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 51]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 0, 2, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 52]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 1, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 53]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_3_people, 2, 0, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 54]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 0, 3, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 55]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 1, 2, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
        for _ in 1:district_households[index, 56]
            ...
            household_conn_ids = collect(Int, agent_id:new_agent_id)
            agents = create_others(
                agent_id, viruses, viral_loads, household_conn_ids, district_people, district_people_households, index_for_4_people, 2, 1, index)
            agent_id = new_agent_id + 1
            append!(all_agents, agents)
        end
