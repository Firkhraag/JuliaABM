...
function plot_work_contacts()
    ...
    for z = 1:num_connection_variants
        for i = 1:num_runs
            observed_num_infected_age_groups_viruses = load(joinpath(@__DIR__, "..", "..", "sensitivity", "contacts", "results_w_$(z + minimum_conn_number - 1).jld"))["observed_cases"] ./ 10072
            for j = 1:num_years
    ...
    for z = 1:num_connection_variants
        for i = 1:num_runs
            rt = load(joinpath(@__DIR__, "..", "..", "sensitivity", "contacts", "results_w_$(z + minimum_conn_number - 1).jld"))["rt"]
            for j = 1:num_years
...
function plot_school_contacts()
    ...
    for z = 1:num_connection_variants
        for i = 1:num_runs
            observed_num_infected_age_groups_viruses = load(joinpath(@__DIR__, "..", "..", "sensitivity", "contacts", "results_s_$(z + minimum_conn_number - 1).jld"))["observed_cases"] ./ 10072
            for j = 1:num_years
                incidence_arr[z, i, j] = sum(sum(observed_num_infected_age_groups_viruses, dims = 3)[:, :, 1], dims = 2)[:, 1][(52 * (j - 1) + 1):(52 * (j - 1) + 52)]
            end
        end
    ...
    for z = 1:num_connection_variants
        for i = 1:num_runs
            rt = load(joinpath(@__DIR__, "..", "..", "sensitivity", "contacts", "results_s_$(z + minimum_conn_number - 1).jld"))["rt"]
            for j = 1:num_years
                rt_arr[z, i, j] = moving_average(rt, 20)[(365 * (j - 1) + 1):(365 * (j - 1) + 365)]
            end
        end
