...
# ╔═╡ 71c84930-7962-11ec-2ec4-b10494c339c3
begin
	using Plots
...
begin
	...
	school_contact_counts_matrix = readdlm(joinpath(
            @__DIR__, "..", "..", "..", "output", "tables", "contacts", "contact_counts_activity_3.csv"), ',', Float64)
	school_contact_counts_matrix += readdlm(joinpath(
	...
	work_contact_counts_matrix = readdlm(joinpath(
            @__DIR__, "..", "..", "..", "output", "tables", "contacts", "contact_counts_activity_4.csv"), ',', Float64)

        home_contact_counts = zeros(Float64, 18, 18)
        for i = 1:18
            for j = 1:18
                for k = 0:4
                    for l = 0:4
                        home_contact_counts[i, j] += home_contact_counts_matrix[(i - 1) * 5 + 1 + k, (j - 1) * 5 + 1 + l]
                    end
                end
				if home_contact_counts[i, j] < 0.0001
					home_contact_counts[i, j] += 0.0001
				end
            end
        end
		...
		for i = 1:18
			...
			home_contact_counts[:, i] ./= num_people
		end

		school_contact_counts = zeros(Float64, 18, 18)
        for i = 1:18
            for j = 1:18
                for k = 0:4
                    for l = 0:4
                        school_contact_counts[i, j] += school_contact_counts_matrix[(i - 1) * 5 + 1 + k, (j - 1) * 5 + 1 + l]
                    end
                end
				if school_contact_counts[i, j] < 0.0001
					school_contact_counts[i, j] += 0.0001
				end
            end
        end
		...
		for i = 1:18
			...
			school_contact_counts[:, i] ./= num_people
		end

		work_contact_counts = zeros(Float64, 18, 18)
        for i = 1:18
            for j = 1:18
                for k = 0:4
                    for l = 0:4
                        work_contact_counts[i, j] += work_contact_counts_matrix[(i - 1) * 5 + 1 + k, (j - 1) * 5 + 1 + l]
                    end
                end
				if work_contact_counts[i, j] < 0.0001
					work_contact_counts[i, j] += 0.0001
				end
            end
        end
		...
		for i = 1:18
			...
			work_contact_counts[:, i] ./= num_people
		end
end

# ╔═╡ 85282e36-490e-4691-990c-089fca43a8e9
begin
	ticks = collect(1:16)
    ticklabels = [
...
begin
	sum_home_contacts = zeros(Float64, 16)
	for i = 1:16
		sum_home_contacts[i] = sum(home_contacts[:, i])
	end
	
	sum_school_contacts = zeros(Float64, 16)
	for i = 1:16
		sum_school_contacts[i] = sum(school_contacts[:, i])
	end
	...
	sum_school_contact_counts = zeros(Float64, 16)
	for i = 1:16
		sum_school_contact_counts[i] = sum(school_contact_counts[:, i])
	end

	sum_work_contact_counts = zeros(Float64, 16)
	for i = 1:16
		sum_work_contact_counts[i] = sum(work_contact_counts[:, i])
	end
...
begin
	labels = CategoricalArray(repeat(["0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79"], outer = 3))
    levels!(labels, ["0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79"])
...
begin
	mean2 = copy(sum_school_contact_counts)
	append!(mean2, sum_school_contacts)
...
begin
	mean3 = copy(sum_work_contact_counts)
	append!(mean3, sum_work_contacts)
...
# ╔═╡ fe5b43d7-b1dc-4f16-be97-61c633352c59
heatmap(
        home_contacts,
        fontfamily = "Times",
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "e1f04b35b107675d19c326d23bb828e0a8d18479"
